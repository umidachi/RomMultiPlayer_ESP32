...
void displaySpriteImage(int index,const std::vector<uint8_t>compressed) {
...
void setup() {
   ...
   for (uint8_t i = 0; i < 4; i++) {
    ...
    //ポケモンのスプライトデータポインタから、圧縮されたデータを取得する 
    //uint8_t dex_id=151;
    uint32_t SpritePointerAddress = 0x383de;  //general pokemon address

    stopByte = {};

    std::vector<uint8_t> general_pokemon=readROMData(romPath, SpritePointerAddress + 28*(dex_id-1) , 28, stopByte);
    //std::vector<uint8_t>buf(general_pokemon.begin()+11 , dex_detail.begin() + 12);
    uint32_t sprite_address = readLittleEndian16(general_pokemon,11);   //11要素目から2バイト分をリトルエンディアンで読み込み

    uint8_t bank =getPokemonSpriteBank(dex_to_index[dex_id]);   //ポケモンのインデックスからBANKを取得

    sprite_address = (sprite_address - 0x4000) + bank * 0x4000;  
    //抽出したアドレスから圧縮されたスプライトデータを抽出する
    stopByte = {};
    std::vector<uint8_t> commpressed_sprite=readROMData(romPath, sprite_address, 700, stopByte);
    ...
    Serial.print("sprite_address:0 "); Serial.println(sprite_address);
    Serial.print("compressed:0 "); Serial.println(commpressed_sprite[0]);
    ...
    //drawBinaryString(tft, dex_text_data, 0, 150, 2, 1, romPath);
      // 最初の画像を表示
    displaySpriteImage(currentImage,commpressed_sprite);
}
